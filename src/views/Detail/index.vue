    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <!-- 商品售卖属性 -->
          <div class="choose" v-if="spuSaleAttrList">
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- 用户购买数量 -->
                <!-- 这个输入框可能用户会输入,可以给这个输入框绑定一个change事件,监测输入的内容是否合法 -->
                <input
                  autocomplete="off"
                  class="itxt"
                  v-model="buyNum"
                ...
                <a href="javascript:" class="mins" @click="buyNum--">-</a>
              </div>
              <div class="add">
...
export default {
  ...
  mounted() {
    // 派发请求,获得商品详情数据
    this.$store.dispatch("detailList", this.$route.params.goodsId);
  },
  computed: {
    // 获取getters里面的数据
  ...
  methods: {
    /**
     * @description:
     * @param {*} selected -- 当前选中的售卖属性的某个属性对象(颜色啊还是版本啊的哪个属性对象)
     * @param {*} allList -- 当前选中的售卖属性的所有对象
     * @return {*}
     */
    changeCheck(selected, allList) {
      // 排他思想,让所选属性种类的其它属性值全部未选中
    ...
    checkInput(event) {
      // 正整数正则
      let reg = /^[1-9]+$/;
      let value = parseInt(event.target._value);
      if (!reg.test(value)) {
        this.buyNum = 1;
      } else {
        this.buyNum = value;
    ...
    async addCart() {
      try {
        ...
        // 通过判断dispatch的返回值的Promise状态执行不同的操作
        await this.$store.dispatch("addOrUpdata", data);
        // 跳转之前,将手机的信息保存再绘画存储里面
